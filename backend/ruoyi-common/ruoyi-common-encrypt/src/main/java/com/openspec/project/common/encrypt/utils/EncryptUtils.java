package com.openspec.project.common.encrypt.utils;
...
public class EncryptUtils {
    ...
    public static String decryptBySm2(String data, String privateKey) {
        ...
        byte[] cipherBytes = HexUtil.decodeHex(data);
        // sm-crypto 输出密文的 C1 省略非压缩点前缀 04，BC 引擎解密前需补全
        if (cipherBytes.length > 0 && cipherBytes[0] != 0x04) {
            cipherBytes = ArrayUtil.addAll(new byte[]{0x04}, cipherBytes);
        }
        return new String(sm2.decrypt(cipherBytes, KeyType.PrivateKey), StandardCharsets.UTF_8);
    }

    /**
     * 将 Base64 编码的 SM2 公钥转换为 sm-crypto 可用的 HEX 公钥（04||x||y 非压缩点）。
     *
     * @param publicKey Base64 编码的公钥
     * @return HEX 编码公钥（130 位十六进制，以 04 开头）
     */
    public static String getSm2PublicKeyHex(String publicKey) {
        if (StrUtil.isBlank(publicKey)) {
            throw new IllegalArgumentException("SM2需要传入公钥");
        }
        try {
            KeyFactory kf = KeyFactory.getInstance("EC", GlobalBouncyCastleProvider.INSTANCE.getProvider());
            BCECPublicKey pub = (BCECPublicKey) kf.generatePublic(new X509EncodedKeySpec(Base64.decode(publicKey)));
            return HexUtil.encodeHexStr(pub.getQ().getEncoded(false));
        } catch (Exception e) {
            throw new IllegalArgumentException("SM2公钥解析失败: " + e.getMessage(), e);
        }
    }
    ...
    public static String sm3Password(String raw) {
        String s = SmUtil.sm3(raw);
        s = SmUtil.sm3(s);
        s = SmUtil.sm3(s);
        return s;
    }
