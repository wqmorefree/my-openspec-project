...
import com.openspec.project.system.domain.bo.SysUserProfileBo;
import com.openspec.project.system.domain.vo.ProfileUserVo;
...
public class SysProfileController extends BaseController {
    ...
    public R<Void> updatePwd(@Validated @RequestBody SysUserPasswordBo bo) {
        ...
        if (!EncryptUtils.sm3Password(bo.oldPassword()).equals(password)) {
            return R.fail("修改密码失败，旧密码错误");
        }
        if (EncryptUtils.sm3Password(bo.newPassword()).equals(password)) {
            return R.fail("新密码不能与旧密码相同");
        }
        int rows = DataPermissionHelper.ignore(() -> userService.resetUserPwd(user.getUserId(), EncryptUtils.sm3Password(bo.newPassword())));
        if (rows > 0) {
            return R.ok();
        }
